...
/* ============ Sample presets ============ */
var PRESETS=[
 {id:'text-compare-overview',name:'文本比较产品功能概览',category:'text',
  left:"文本比较在线差异对比工具 - 概览\n\n文本比较是一款快速、精准的在线文本比较工具，专为开发者、作者、编辑和法律专业人士设计。\n\n核心功能：\n- 即时并排与统一行内差异视图\n- 字符级、词级和行级差异检测\n- 零服务器上传：100% 客户端私密文本比较\n- 行号同步与平滑滚动\n- 内置 JSON 格式化、行排序和行清理\n\n为什么使用在线文本比较？\n逐行手动比较文档既缓慢又容易出错。我们的文本比较引擎实时高亮新增、修改和删除的文本，并配以醒目的颜色提示。",
  right:"文本比较 PRO 在线差异对比工具 - 概览与规格\n\n文本比较 PRO 是一款高性能的在线文本比较工具，专为全球开发者、文案、编辑和法律团队设计。\n\n核心功能：\n- 即时并排分栏与统一行内差异显示\n- 字符级、词级和行级差异检测引擎\n- 100% 客户端隐私：无服务器数据传输或存储\n- 行号对齐、同步平滑滚动与直接跳转锚点\n- 内置 JSON 格式化、字母行排序和空行删除\n- 一键导出统一 Git 补丁（.diff）与 HTML 差异报告下载\n\n为什么选择在线文本比较 PRO？\n逐行手动比较文本文档既费力又容易出错。我们先进的文本比较引擎即时高亮新增、修改和删除内容，配以高对比度颜色指示。"},
 {id:'text-diff-release-notes',name:'文本比较发布说明（Markdown）',category:'markdown',
  left:"# 文本比较工具 v1.0\n\n欢迎使用我们的免费文本比较工具的首个版本！\n\n## 核心功能\n* 在线比较纯文本文档。\n* 分栏视图用于并排文本检查。\n* 基础统计：新增和删除行数。\n\n## 使用指南\n将原文粘贴到左侧面板，修改后的文本粘贴到右侧面板。点击比较查看差异。",
  right:"# 文本比较工具 v2.0（专业版发布）\n\n欢迎使用我们免费的基于浏览器的文本比较工具的重大升级！\n\n## 核心功能\n* 超快在线文本比较引擎，零服务器延迟。\n* 双视图模式：并排分栏视图和单列统一视图。\n* 精细比较模式：字符、词和行级匹配。\n* 实时相似度百分比计算。\n* 快捷操作工具栏：JSON 美化、忽略大小写、修剪空白和行排序。\n\n## 使用与隐私指南\n将原文粘贴到左侧，修改后的文本粘贴到右侧。您的文本绝不会离开您的浏览器！"},
 {id:'text-compare-json-config',name:'文本比较差异设置（JSON）',category:'json',
  left:'{\n  "appName": "文本比较工具",\n  "version": "1.0.0",\n  "settings": {\n    "viewMode": "split",\n    "granularity": "word",\n    "ignoreWhitespace": false,\n    "ignoreCase": false,\n    "showLineNumbers": true\n  },\n  "supportedFormats": ["txt", "md", "json", "code"]\n}',
  right:'{\n  "appName": "文本比较工具 PRO",\n  "version": "2.0.0",\n  "settings": {\n    "viewMode": "split",\n    "granularity": "word",\n    "ignoreWhitespace": true,\n    "ignoreCase": true,\n    "ignoreLineEndings": true,\n    "showLineNumbers": true,\n    "syncScroll": true,\n    "onlyShowDifferences": false\n  },\n  "supportedFormats": ["txt", "md", "json", "js", "ts", "html", "diff"],\n  "seoOptimized": true\n}'},
 {id:'text-compare-algorithm-code',name:'文本差异算法（TypeScript）',category:'code',
...
function renderStats(){
  $('statMatch').textContent=stats.similarity+'% 匹配';
  $('statAdd').textContent=stats.additions+' 新增';
  $('statDel').textContent=stats.deletions+' 删除';
  $('statUnch').textContent=stats.unchanged+' 未变';
  var lbl=$('stepperLbl');
  if(stats.diffCount>0)lbl.innerHTML='第 <strong>'+state.currentDiffIndex+'</strong> 处差异，共 <strong>'+stats.diffCount+'</strong> 处';
  else lbl.textContent='无差异';
...
function renderEditStats(){
  ...
  $('leftStat').textContent=lc+' 行 • '+(state.leftText?state.leftText.length:0)+' 字符';
  $('rightStat').textContent=rc+' 行 • '+(state.rightText?state.rightText.length:0)+' 字符';
  $('leftLines').textContent=splitData.filter(function(l){return l.leftLineNum!==undefined;}).length+' 行';
  $('rightLines').textContent=splitData.filter(function(l){return l.rightLineNum!==undefined;}).length+' 行';
}
...
function applyTheme(){
  ...
  var tb=$('themeBtn');if(tb){tb.querySelector('span').textContent=state.theme==='dark'?'浅色':'深色';tb.querySelector('.ic').innerHTML=ICONS[state.theme==='dark'?'sun':'moon'];}
...
/* ============ Edit panel actions ============ */
function fmtJson(side){var tgt=side==='left'?state.leftText:state.rightText;try{var o=JSON.parse(tgt);setSide(side,JSON.stringify(o,null,2));}catch(e){alert((side==='left'?'左侧':'右侧')+'面板中的 JSON 无效');}}
...
function copyPanel(side){
  ...
  if(btn){btn.classList.add('copied');var sp=btn.querySelector('span');var old=sp.textContent;sp.textContent='已复制';setTimeout(function(){btn.classList.remove('copied');sp.textContent=old;},2000);}
}
function copyAll(){
  var combined='=== 原文 ===\n'+state.leftText+'\n\n=== 修改 ===\n'+state.rightText;
...
function dlHtml(){
  ...
  var html='<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>文本比较差异报告</title><style>body{font-family:system-ui,sans-serif;background:'+bg+';color:'+tc+';padding:2rem 1rem;margin:0;line-height:1.5}.container{max-width:960px;margin:0 auto}.card{background:'+card+';border-radius:12px;padding:1.5rem;box-shadow:0 4px 20px rgba(0,0,0,.08);border:1px solid '+bc+'}h1{margin-top:0;font-size:1.5rem;color:#6366f1}.stats{display:flex;flex-wrap:wrap;gap:.75rem;margin-bottom:1.5rem;font-family:monospace;font-size:.85rem;font-weight:600}.badge{padding:.35rem .85rem;border-radius:9999px}.similarity{background:rgba(99,102,241,.15);color:#6366f1}.added{background:rgba(16,185,129,.15);color:#10b981}.removed{background:rgba(244,63,94,.15);color:#f43f5e}.diff-box{background:'+cb+';padding:1rem;border-radius:8px;overflow-x:auto;font-family:monospace;font-size:.85rem;line-height:1.6;border:1px solid '+bc+'}.line{white-space:pre-wrap;word-break:break-all;padding:.1rem .4rem;border-radius:2px}.line.add{background:rgba(16,185,129,.2);color:'+ac+'}.line.del{background:rgba(244,63,94,.2);color:'+dc+'}.line.chunk-hdr{color:#818cf8;font-weight:bold;background:rgba(99,102,241,.1);margin:.4rem 0}</style></head><body><div class="container"><div class="card"><h1>文本比较差异报告</h1><div class="stats"><span class="badge similarity">'+stats.similarity+'% 匹配</span><span class="badge added">+'+stats.additions+' 新增</span><span class="badge removed">-'+stats.deletions+' 删除</span></div><div class="diff-box">'+lines+'\n</div></div></div></body></html>';
...
function init(){
  ...
  var sc=[['Alt + N','跳转到下一处差异'],['Alt + P','跳转到上一处差异'],['Alt + S','交换原文与修改文本'],['Alt + E','打开导出弹窗'],['Alt + C','清空两侧文本'],['Alt + H','打开快捷键与帮助']];
